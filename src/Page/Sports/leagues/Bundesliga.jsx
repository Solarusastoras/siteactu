...
const Bundesliga = ({ view = 'matches' }) => {
  ...
  const [allWeekendMatches, setAllWeekendMatches] = useState(() => {
    const cached = localStorage.getItem('bundesliga_weekend_matches');
    return cached ? JSON.parse(cached) : [];
  });
  const [upcomingWeekendMatches, setUpcomingWeekendMatches] = useState(() => {
    const cached = localStorage.getItem('bundesliga_upcoming_matches');
    return cached ? JSON.parse(cached) : [];
  });
  const [standings, setStandings] = useState(currentBundesligaStandings);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const lastUpdate = localStorage.getItem('bundesliga_last_update');
        ...
        const forceUpdate = !localStorage.getItem('bundesliga_upcoming_matches') || JSON.parse(localStorage.getItem('bundesliga_upcoming_matches') || '[]').length === 0;
        const currentTime = hours * 60 + now.getMinutes();
        const isMatchTime = 
          (dayOfWeek === 5 && currentTime >= 20 * 60 + 29 && currentTime <= 23 * 60 + 20) || // Vendredi 20h29-23h20
          (dayOfWeek === 6 && currentTime >= 15 * 60 + 29 && currentTime <= 21 * 60) || // Samedi 15h29-21h00
          (dayOfWeek === 0 && currentTime >= 15 * 60 + 29 && currentTime <= 20 * 60); // Dimanche 15h29-20h00
        
        if (isMatchTime) {
          const matchesResponse = await fetch('https://site.api.espn.com/apis/site/v2/sports/soccer/ger.1/scoreboard');
        ...
        if (shouldFetchWeekend || neverFetched || isOutdated || forceUpdate) {
          ...
          try {
            ...
            weekends.forEach((weekend) => {
              allFetches.push(
                fetch(`https://site.api.espn.com/apis/site/v2/sports/soccer/ger.1/scoreboard?dates=${formatDate(weekend.friday)}`),
                fetch(`https://site.api.espn.com/apis/site/v2/sports/soccer/ger.1/scoreboard?dates=${formatDate(weekend.saturday)}`),
                fetch(`https://site.api.espn.com/apis/site/v2/sports/soccer/ger.1/scoreboard?dates=${formatDate(weekend.sunday)}`)
              );
            });
            ...
            localStorage.setItem('bundesliga_weekend_matches', JSON.stringify(currentWeekendMatches));
            localStorage.setItem('bundesliga_upcoming_matches', JSON.stringify(nextWeekendMatches));
            localStorage.setItem('bundesliga_last_update', now.toISOString());
          } catch (weekendError) {
            console.log('Erreur récupération matchs week-end Bundesliga:', weekendError);
          }
        } else if (!isMatchTime) {
          console.log('📦 Utilisation des matchs Bundesliga en cache');
        }
        
        try {
          const standingsResponse = await fetch('https://site.api.espn.com/apis/v2/sports/soccer/ger.1/standings');
        ...
        console.error('Erreur lors du chargement Bundesliga:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <FootballLeague 
      ...
      leagueConfig={leaguesConfig.bundesliga}
      view={view}
    />
  );
};
