...
const PremierLeague = ({ view = 'matches' }) => {
  ...
  const [allWeekendMatches, setAllWeekendMatches] = useState(() => {
    const cached = localStorage.getItem('premier_weekend_matches');
    return cached ? JSON.parse(cached) : [];
  });
  const [upcomingWeekendMatches, setUpcomingWeekendMatches] = useState(() => {
    const cached = localStorage.getItem('premier_upcoming_matches');
    return cached ? JSON.parse(cached) : [];
  });
  const [standings, setStandings] = useState(currentPremierLeagueStandings);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const lastUpdate = localStorage.getItem('premier_last_update');
        ...
        const forceUpdate = !localStorage.getItem('premier_upcoming_matches') || JSON.parse(localStorage.getItem('premier_upcoming_matches') || '[]').length === 0;
        const currentTime = hours * 60 + now.getMinutes();
        const isMatchTime = 
          (dayOfWeek === 6 && currentTime >= 13 * 60 + 29 && currentTime <= 21 * 60) || // Samedi 13h29-21h00
          (dayOfWeek === 0 && currentTime >= 14 * 60 + 59 && currentTime <= 20 * 60) || // Dimanche 14h59-20h00
          (dayOfWeek === 1 && currentTime >= 20 * 60 + 59 && currentTime <= 23 * 60 + 20) || // Lundi 20h59-23h20
          ((dayOfWeek === 2 || dayOfWeek === 3 || dayOfWeek === 4) && currentTime >= 20 * 60 + 29 && currentTime <= 23 * 60 + 30); // Midweek 20h29-23h30
        
        if (isMatchTime) {
          const matchesResponse = await fetch('https://site.api.espn.com/apis/site/v2/sports/soccer/eng.1/scoreboard');
        ...
        if (shouldFetchWeekend || neverFetched || isOutdated || forceUpdate) {
          ...
          try {
            ...
            weekends.forEach((weekend) => {
              allFetches.push(
                fetch(`https://site.api.espn.com/apis/site/v2/sports/soccer/eng.1/scoreboard?dates=${formatDate(weekend.friday)}`),
                fetch(`https://site.api.espn.com/apis/site/v2/sports/soccer/eng.1/scoreboard?dates=${formatDate(weekend.saturday)}`),
                fetch(`https://site.api.espn.com/apis/site/v2/sports/soccer/eng.1/scoreboard?dates=${formatDate(weekend.sunday)}`)
              );
            });
            ...
            localStorage.setItem('premier_weekend_matches', JSON.stringify(currentWeekendMatches));
            localStorage.setItem('premier_upcoming_matches', JSON.stringify(nextWeekendMatches));
            localStorage.setItem('premier_last_update', now.toISOString());
          } catch (weekendError) {
            console.log('Erreur récupération matchs week-end Premier League:', weekendError);
          }
        } else if (!isMatchTime) {
          console.log('📦 Utilisation des matchs Premier League en cache');
        }
        
        try {
          const standingsResponse = await fetch('https://site.api.espn.com/apis/v2/sports/soccer/eng.1/standings');
        ...
        console.error('Erreur lors du chargement Premier League:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <FootballLeague 
      ...
      leagueConfig={leaguesConfig.premier}
      view={view}
    />
  );
};
