...
import { serieaConfig } from '../data/leaguesConfig';
import { currentSerieAStandings } from '../data/standingsData';
...
const SerieA = ({ view = 'matches' }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const serieaData = jsonData.sports?.seriea;
        
        setData(serieaData);
        setLoading(false);
      } catch (err) {
        console.error('Erreur chargement Serie A:', err);
        setLoading(false);
      }
    };
  ...
  const getPositionClass = (position) => {
    for (const rule of serieaConfig.positionRules) {
  ...
  if (loading) {
    return <div className="loading"><h2>Chargement Serie A...</h2></div>;
  }
  ...
  const standings = calculateStandings(allMatches, currentSerieAStandings);
    ...
    return (
      <div className="football-league">
        <div className="league-header">
          <h2>{serieaConfig.icon} {serieaConfig.name}</h2>
        </div>

        <div className="games-grid">
          {allMatches.length === 0 ? (
            <div className="game-card">
              <p>Aucun match Serie A disponible</p>
            </div>
          ) : (
            allMatches.map((game) => {
  ...
  if (view === 'classement') {
    return (
      <div className="football-league">
        <div className="league-header">
          <h2>{serieaConfig.icon} Classement {serieaConfig.name}</h2>
        </div>

        <div className="legend-container">
          {serieaConfig.legend.map((item, index) => (
