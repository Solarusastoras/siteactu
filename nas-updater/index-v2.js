...
const CONFIG = {
  ...
  newsUpdateInterval: parseInt(process.env.NEWS_UPDATE_INTERVAL) || 3600000, // 1 heure pour actus/culture
...
function processStandings(standingsData, sportType = 'football') {
  // Pour les sports américains (NFL, NBA, NHL), la structure est différente
  if (standingsData?.entries && Array.isArray(standingsData.entries)) {
    // Structure directe avec entries (une seule conférence ou ligue)
    return standingsData.entries.map(entry => ({
      ...
      stats: entry.stats?.reduce((acc, stat) => {
        ...
        acc[stat.abbreviation] = stat.displayValue || stat.value;
        return acc;
      }, {})
    }));
  }
  
  // Structure pour football européen (Ligue 1, Premier League, etc.)
...
async function fetchESPNCompetition(key, competition) {
  try {
    ...
    // Extraire les standings selon la structure de l'API
    let standingsData = [];
    
    // Pour les sports américains avec conférences (NFL, NBA, NHL)
    if (standingsRes.data.children && Array.isArray(standingsRes.data.children)) {
      // Chaque enfant est une conférence (AFC/NFC, Eastern/Western, etc.)
      standingsData = standingsRes.data.children.map(conference => ({
        name: conference.name,
        abbreviation: conference.abbreviation,
        standings: processStandings(conference.standings)
      }));
    }
    // Pour les ligues simples (Ligue 1, Premier League, etc.)
    else if (standingsRes.data.standings) {
      standingsData = processStandings(standingsRes.data.standings);
    }
    else if (Array.isArray(standingsRes.data)) {
      standingsData = processStandings(standingsRes.data);
    }
...
async function connectFTP() {
  const client = new ftp.Client(30000); // Timeout de 30 secondes
  client.ftp.verbose = true; // Toujours verbose pour débugger
  try {
    await client.access({
    ...
    log(`✗ FTP Config: ${CONFIG.ftp.host}:${CONFIG.ftp.port} user=${CONFIG.ftp.user}`, 'error');
    throw error;
  }
}
...
async function uploadAllFiles(results) {
  ...
  try {
    ...
    // Lire le fichier existant s'il existe, sinon créer une structure vide
    ...
    try {
      const existingContent = await fs.readFile(consolidatedPath, 'utf8');
      const existingData = JSON.parse(existingContent);
      // Conserver TOUTES les données existantes
      consolidatedData = {
        sports: existingData.sports || {},
        actus: existingData.actus || {},
        culture: existingData.culture || {},
        mercato: existingData.mercato || {},
        lastUpdate: existingData.lastUpdate || new Date().toISOString()
      };
    } catch (err) {
      // Fichier n'existe pas ou erreur de lecture, on continue avec la structure vide
    }

    // Lire et consolider tous les nouveaux fichiers JSON
    ...
    consolidatedData.lastUpdate = new Date().toISOString();
