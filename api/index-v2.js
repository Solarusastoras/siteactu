...
const CONFIG = {
  ...
  newsUpdateInterval: parseInt(process.env.NEWS_UPDATE_INTERVAL) || 3600000, // 1 heure pour actus/culture/mercato
  outputDir: path.join(__dirname, 'output'),
  debug: process.env.DEBUG === 'true'
};

// API alternative pour les standings - utilise l'API site ESPN
const STANDINGS_API = {
...
function processLiveMatches(events) {
  ...
  // Retourner la structure ESPN complète sans transformation
  return events;
}
...
function processStandings(standings) {
  ...
  // Récupérer le premier standing (généralement le classement général)
  const mainStanding = standings[0];
  if (!mainStanding || !mainStanding.entries) return [];
  
  // Retourner directement les entries avec la structure complète ESPN
  return mainStanding.entries.map(entry => ({
    team: {
      id: entry.team?.id,
...
// Récupérer les standings depuis l'API site ESPN
async function fetchAlternativeStandings(leagueKey) {
  try {
    const leagueId = STANDINGS_API.leagueIds[leagueKey];
    if (!leagueId) return null;

    // Utiliser l'endpoint site.web.api.espn.com/apis/v2/sports/soccer/{league}/standings
    const url = `${STANDINGS_API.baseUrl}/${leagueId}/standings`;
    console.log(`🔍 Fetching alternative standings: ${url}`);
    ...
    console.log(`📦 Alternative API response structure:`, Object.keys(response.data));
    ...
    } else if (response.data.children && response.data.children[0]?.standings) {
      standingsData = response.data.children[0].standings[0];
    } else if (response.data.data?.standings) {
      standingsData = response.data.data.standings;
    }

    if (!standingsData || !standingsData.entries) {
      console.log(`❌ No standings entries found in alternative API`);
      return null;
    }

    console.log(`✓ Found ${standingsData.entries.length} teams in alternative standings`);
    
    // Utiliser processStandings existant
    return standingsData.entries.map(entry => ({
      team: {
        id: entry.team?.id,
        displayName: entry.team?.displayName,
        shortDisplayName: entry.team?.shortDisplayName,
    ...
    console.error(`❌ Erreur standings alternatifs ${leagueKey}:`, error.message);
    if (error.response) {
...
async function fetchESPNCompetition(key, competition) {
  try {
    // Toujours fetch scoreboard et standings séparément avec meilleur gestion d'erreur
    const scoreboardRes = await axios.get(competition.scoreboard, { timeout: 15000 });
    
    let standings = [];
    
    // Essayer ESPN standings d'abord
    try {
      const standingsRes = await axios.get(competition.standings, { timeout: 15000 });
      console.log(`📊 ${competition.name} - Réponse standings ESPN:`, JSON.stringify(standingsRes.data).substring(0, 200));
      
      if (standingsRes.data && standingsRes.data.children && standingsRes.data.children.length > 0) {
        const childStandings = standingsRes.data.children[0].standings;
        standings = processStandings(childStandings || []);
        console.log(`✅ ${competition.name} - ${standings.length} équipes depuis ESPN`);
      }
    } catch (espnError) {
      console.log(`⚠️ ${competition.name} - ESPN standings échoué:`, espnError.message);
    }

    // Si pas de standings ESPN, utiliser l'API alternative pour les ligues de foot
    if (standings.length === 0 && STANDINGS_API.leagueIds[key]) {
      console.log(`📊 ${competition.name} - Tentative API alternative...`);
      const altStandings = await fetchAlternativeStandings(key);
      if (altStandings && altStandings.length > 0) {
        standings = altStandings;
        console.log(`✅ ${competition.name} - ${standings.length} équipes via API alternative`);
      } else {
        console.log(`❌ ${competition.name} - Aucun classement disponible`);
      }
    }
  ...
  try {
    ...
    for (const result of results) {
      ...
      if (filename.startsWith('actus-')) {
        ...
        // Pour les sports, utiliser le nom du fichier sans extension comme clé
...
async function sportsLoop() {
  ...
  // Mise à jour quotidienne à 5h00 pour toutes les ligues de football (scores + standings du jour précédent)
  const isFootballDailyUpdate = true; // TOUJOURS actif pour avoir les données en permanence
  ...
  if (isFootballDailyUpdate) {
    log('=== 🕐 5h00: Mise à jour quotidienne toutes les ligues de football ===');
  }
  ...
  try {
    const competitionsToUpdate = Object.entries(ESPN_COMPETITIONS).filter(([key, comp]) => {
      ...
      // Ligues de football: mise à jour quotidienne à 5h00
      if (isFootballDailyUpdate && (key === 'ligue1' || key === 'ligue2' || key === 'premierLeague' || 
          key === 'laLiga' || key === 'serieA' || key === 'bundesliga' || key === 'brasileirao' || 
          key === 'can' || key === 'championsLeague')) {
        return true;
      }
